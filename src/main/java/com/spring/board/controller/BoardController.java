...
//	@RequestMapping(value="/test/test_form.action", method= {RequestMethod.GET})  // 오로지 GET 방식만 허락하는 것임.
//	@RequestMapping(value="/test/test_form.action", method= {RequestMethod.POST}) // 오로지 POST 방식만 허락하는 것임. 
	@RequestMapping(value="/test/test_form.action") // GET 방식 및 POST 방식 둘 모두 허락하는 것임. 
	...
	@RequestMapping(value="/test/test_form_2.action", method= {RequestMethod.GET})  // 오로지 GET 방식만 허락하는 것임.
	...
	@RequestMapping(value="/test/test_form_2.action", method= {RequestMethod.POST})  // 오로지 POST 방식만 허락하는 것임.
	public String test_form_2(TestVO vo) {
	
	/*	
		System.out.println("~~~ 확인용 vo.getNo() : " + vo.getNo());
		System.out.println("~~~ 확인용 vo.getIrum() : " + vo.getIrum());
		~~~ 확인용 vo.getNo() : 104
        ~~~ 확인용 vo.getIrum() : 김성민
    */	
	...
	// ========== Ajax 연습시작  ========== //
	@RequestMapping(value="/test/test_form_3.action", method= {RequestMethod.GET})  // 오로지 GET 방식만 허락하는 것임.
	...
	*/
	@ResponseBody
	@RequestMapping(value="/test/ajax_select.action", method= {RequestMethod.GET}, produces="text/plain;charset=UTF-8")
	...
	public String ajax_insert(TestVO vo) {
	...
	@RequestMapping(value="/test/test_form_modelandview.action") // GET 방식 및 POST 방식 둘 모두 허락하는 것임. 
	public ModelAndView test_form(HttpServletRequest request, TestVO vo, ModelAndView mav) {
		
		String method = request.getMethod();
		
	    if("GET".equalsIgnoreCase(method)) { // "GET" 방식이라면 
	    	mav.setViewName("test/test_form");
	    	//  /WEB-INF/views/test/test_form.jsp 페이지를 만들어야 한다.
	    }
	    	...
	    	if(n==1) {
	    		...
	    		mav.setViewName("redirect:/test/test_form_modelandview.action");
	    	    //   /test/test_form_modelandview.action 페이지로 redirect(페이지이동)해라는 말이다.
	    	}
    ...
    //	request.setAttribute("testvoList", testvoList);
		...
		mav.setViewName("test/test_select");
	//	return "test/test_select";
	    //   /WEB-INF/views/test/test_select.jsp 페이지를 만들어야 한다.
	...
	// === #40. 로그인 폼 페이지 요청 === //
	@RequestMapping(value="/login.action", method= {RequestMethod.GET})  // 오로지 GET 방식만 허락하는 것임. 
	...
	// === #41. 로그인 처리하기 === //
	@RequestMapping(value="/loginEnd.action", method= {RequestMethod.POST})  // 오로지 POST 방식만 허락하는 것임. 
